...
Скачайте `.zip` из [последнего релиза](https://github.com/AppsGanin/WashMyMac/releases/latest),
распакуйте и перетащите `WashMyMac.app` в `/Applications`.

> [!NOTE]
> Приложение не подписано сертификатом Apple Developer, поэтому первый запуск macOS
> заблокирует. Правый клик по приложению → **Открыть** → **Открыть** ещё раз. Один раз.
